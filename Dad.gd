...
func take_damage(amount):
	...
	if health <= 0:
		die()
	elif not transformed and health <= transform_stage:
		transform()
	else:
...
func _on_AttackCooldown_timeout():
	if player and is_instance_valid(player) and alive and not transformed:
		if global_position.distance_to(player.global_position) <= attack_range + attack_buffer:
			if not attack_in_progress:
				attack_in_progress = true
				animation_locked = true
				var attack_type = randi() % 3 + 1
				anim.play("attack%d_%s" % [attack_type, anim_direction])
		else:
			cooldown.start()
...
func transform():
	...
	yield(anim, "animation_finished")
	animation_locked = false
	# optionally change stats here:
	# speed += 30
	# damage *= 1.5
	# or switch to second attack logic
